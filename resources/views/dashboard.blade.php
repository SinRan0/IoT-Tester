...
<head>
    ...
    <style>
        ...
        .value { font-size: 40px; margin-top: 10px; font-weight: bold; color: #38bdf8; }
        .label { font-size: 16px; opacity: 0.8; text-transform: uppercase; letter-spacing: 1px; }
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="value">
            {{ isset($data->soil_moisture) ? round(($data->soil_moisture / 4095) * 100) : 0 }}%
        </div>
    </div>

    <!-- STATUS TANAH -->
    <div class="card">
        <div class="label">Kondisi Tanah</div>
        <div class="value" style="font-size:22px;">
            @if(isset($data->soil_moisture))
                @if($data->soil_moisture > 3000)
                    Kering 🥵
                @elseif($data->soil_moisture > 1500)
                    Normal 🌿
                @else
                    Basah 💧
                @endif
